...
def first_come_first_serve(task_files, total_runtime=120):
    """
    First-Come, First-Serve (FCFS) Scheduler for GPU-aware task allocation for a single GPU
    ...
    print("Starting First-Come, First-Serve Scheduling for a Single GPU...")
...
def _create_gpu_usage_visualization(gpu_usage_history, task_allocation_log):
    ...
    # Save the figure
    plt.savefig('gpu_scheduling_analysis_fcfs.png')
    plt.close()
    
    # Create a detailed CSV log
    df = pd.DataFrame(task_allocation_log)
    df.to_csv('task_allocation_log_fcfs.csv', index=False)
    
    print("Visualization saved as 'gpu_scheduling_analysis_fcfs.png'")
    print("Task allocation log saved as 'task_allocation_log_fcfs.csv'")

# Run the scheduler
try:
    # You can adjust these parameters as needed
    gpu_usage, task_log = first_come_first_serve(task_files, total_runtime=120)
